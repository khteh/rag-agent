...
async def test_email_parser_chain_email0(EmailRAGFixture):
    ...
    config = RunnableConfig(run_name=test_email_parser_chain_email0.__name__, configurable={"thread_id": uuid7str()})
...
async def test_email_parser_chain_email3(EmailRAGFixture):
    ...
    config = RunnableConfig(run_name=test_email_parser_chain_email3.__name__, configurable={"thread_id": uuid7str()})
    result: EmailRAGState = await EmailRAGFixture.ParseEmail(state, config)
    #print(f"extract: {result['extract']}")
    assert result
    assert result["extract"]
