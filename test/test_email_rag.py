...
async def test_email_should_escalate(EmailRAGFixture):
    ...
    assert "several safety violations at the construction site" in ai_message.content
    ...
    assert "and not wearing proper PPE." in ai_message.content
...
async def test_email_should_not_escalate(EmailRAGFixture):
    ...
    assert not ai_message.tool_calls
    assert ai_message.content
    assert "this issue does not" in ai_message.content
